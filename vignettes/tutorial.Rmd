...
Same dataset if choosing a higher nstart value (will sacrifice effiency for robustness):
```{r}
output = k_means_cluster(x, k, nstart = 25)
plot(output$data, col = output$clusters, main = "plot of clustering with high nstart on data", xlab = "output$data[,1]", 
     ylab = "output$data[,2]")
```

Same dataset if choosing a small nstart value (will sacrifice robustness, not recommended):
